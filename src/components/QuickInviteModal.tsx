...
export function QuickInviteModal({ businessId, businessSlug }: QuickInviteModalProps) {
  ...
  const inviteUrl = result?.token ? `${window.location.origin}/escribir-opinion/${encodeURIComponent(businessSlug)}` : '';
  ...
  return <>
    ...
    <dialog ref={dialog} aria-labelledby="invite-title" aria-describedby="invite-description" onCancel={() => trigger.current?.focus()} className="m-auto max-h-[90dvh] w-[calc(100%_-_2rem)] max-w-lg overflow-y-auto rounded-2xl border border-op-border bg-op-sheet p-0 text-op-ink shadow-2xl backdrop:bg-black/40">
      <div className="flex items-start justify-between gap-4 border-b border-op-border p-5">
        <div><h2 id="invite-title" className="text-xl font-semibold">Invitar a un cliente</h2><p id="invite-description" className="mt-1 text-sm leading-relaxed text-op-muted">Registra la invitación y comparte el formulario público de opiniones.</p></div>
