    ...
    <section id="metodologia" className="border-y border-op-border bg-op-shaded">
      <div className="op-container op-section">
        ...
        <div className="mt-10 grid gap-8 md:grid-cols-3 md:gap-10">
          {[
            { icon: Fingerprint, number: '01', title: 'Existe', subtitle: 'Conoce quién está detrás.', body: 'Consulta la razón social, los canales de contacto y el estado de verificación de cada identidad. Un dato público y uno verificado se muestran por separado.' },
            { icon: ReceiptText, number: '02', title: 'Cumple', subtitle: 'Lee opiniones con contexto.', body: 'Revisa el tipo de evidencia de cada opinión. Cuando hay pedidos conectados, la cobertura indica qué porcentaje de compradores recibió una invitación a opinar.' },
            { icon: MessagesSquare, number: '03', title: 'Resuelve', subtitle: 'Mira qué pasa después.', body: 'Consulta las respuestas y los casos. Una solución propuesta por el comercio se distingue de una resolución confirmada por quien compró.' },
          ].map(({ icon: Icon, number, title, subtitle, body }) => <div key={number} className="border-t border-op-strong pt-6"><div className="mb-6 flex items-center justify-between"><Icon aria-hidden="true" className="size-6 text-op-green" /><span className="font-mono text-xs text-op-muted">{number}</span></div><h3 className="text-xl font-semibold tracking-tight">{title}</h3><p className="mb-3 mt-2 text-sm font-medium">{subtitle}</p><p className="text-sm leading-relaxed text-op-secondary">{body}</p></div>)}
        </div>
