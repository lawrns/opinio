...
export function SettingsManager({ business, identities: passedIdentities }: SettingsManagerProps) {
  ...
  const [staff, setStaff] = React.useState([
    { id: 1, name: 'Dirección Comercial', email: 'admin@comercio.mx', role: 'Administrador', lastLogin: 'Hoy, 09:42 am', status: 'Activo' },
    { id: 2, name: 'Soporte Conciliación SLA', email: 'atencion@comercio.mx', role: 'Conciliador SLA', lastLogin: 'Ayer, 04:15 pm', status: 'Activo' },
  ]);
  ...
  const handleSearchDenue = () => {
    ...
    setTimeout(() => {
      ...
      setDenueFound('Coincidencia INEGI encontrada: CLEE 0901547891234001 • Establecimiento en operación activo.');
    }, 1200);
  };
  ...
  const identities = passedIdentities && passedIdentities.length > 0 ? passedIdentities : [
    { id: 1, type: 'rfc', identifier: rfc || 'CZE150414AB2', status: 'verified' },
    { id: 2, type: 'denue', identifier: clee ? `CLEE: ${clee}` : 'CLEE: 0901547891234001', status: 'verified' },
    { id: 3, type: 'whatsapp', identifier: whatsapp || '+52 55 4164 0533', status: 'verified' },
    { id: 4, type: 'domain', identifier: domain || 'luuna.mx', status: 'verified' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 p-6 md:p-8 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs space-y-6">
          <div className="border-b border-[#E2E8F0] pb-4">
            ...
            <p className="text-xs text-[#64748B] mt-1">
              ...
              <strong className="text-[#0F172A]">{business.brand_name}</strong> ante el SAT, INEGI y PROFECO.
            </p>
          </div>
          ...
          <form onSubmit={handleSaveSettings} className="space-y-4 text-xs">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-[#FAFAF8] border border-[#E2E8F0] text-xs text-[#0F172A] focus:outline-none focus:border-emerald-500"
              />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block font-medium text-[#0F172A]">
                  Registro Federal de Contribuyentes (RFC)
                </label>
                <span className="text-[10px] text-emerald-700 font-semibold flex items-center gap-1">
                  <CheckCircle2 className="h-3 w-3 text-emerald-600" /> Cédula Fiscal Validada
                </span>
              </div>
              <input
                type="text"
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-[#FAFAF8] border border-[#E2E8F0] text-xs text-[#0F172A] font-mono uppercase focus:outline-none focus:border-emerald-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-[#FAFAF8] border border-[#E2E8F0] text-xs text-[#0F172A] font-mono focus:outline-none focus:border-emerald-500"
              />
              {denueFound && (
                <div className="mt-1.5 p-2 rounded-lg bg-emerald-50 border border-emerald-200 text-[10px] text-emerald-800 flex items-center gap-1.5">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="p-6 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs space-y-4">
            ...
            <div className="space-y-2.5 text-xs">
              {identities.map((idnt) => (
                <div
                  key={idnt.id}
                  className="p-3 rounded-xl bg-[#FAFAF8] border border-[#E2E8F0] flex items-center justify-between"
                >
                  <div className="space-y-0.5">
                    <div className="font-semibold text-[#0F172A] uppercase text-[10px] tracking-wider">
                      {idnt.type === 'rfc'
                        ? 'SAT Cédula Fiscal'
                        : idnt.type === 'denue'
                        ? 'INEGI DENUE'
                    ...
                    <div className="font-mono text-[#64748B] text-[11px] truncate max-w-[200px]">
                      {idnt.identifier}
                    </div>
                  </div>
                  ...
                  <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 flex items-center gap-1">
                    <CheckCircle2 className="h-3 w-3 text-emerald-600" /> Validado
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="p-5 rounded-2xl bg-[#F8FAFC] border border-[#E2E8F0] space-y-2 text-xs">
      ...
      <div className="p-6 md:p-8 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#E2E8F0] pb-4">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-[#FAFAF8] hover:bg-[#F1F5F9] text-[#0F172A] border border-[#CBD5E1] transition-colors"
          >
        ...
        {showAddStaff && (
          <form
            ...
            className="p-4 rounded-xl bg-[#FAFAF8] border border-emerald-300 space-y-3 text-xs animate-in fade-in duration-100"
          >
            <div className="font-semibold text-[#0F172A]">Agregar Nuevo Miembro al Equipo</div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <input
                ...
                className="px-3 py-2 rounded-lg bg-white border border-[#CBD5E1] text-xs text-[#0F172A]"
              />
              <input
                type="email"
                required
                ...
                <tr key={member.id} className="hover:bg-[#F8FAFC] transition-colors">
                  ...
                  <td className="py-2.5 px-3">
                    <span className="text-[10px] font-semibold text-emerald-700 flex items-center gap-1">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600" /> {member.status}
                    </span>
                  </td>
                </tr>
              ))}
