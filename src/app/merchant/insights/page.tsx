...
export default async function MerchantInsightsPage({ searchParams }: { searchParams: Promise<{ business?: string }> }) {
  ...
  return <div className="mx-auto max-w-7xl space-y-8 pb-12">
    ...
    <dl className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map((metric) => <div key={metric.label} className="rounded-2xl border border-op-border bg-op-sheet p-5"><dt className="text-sm font-medium text-op-secondary">{metric.label}</dt><dd className="mt-3 font-data text-3xl font-semibold text-op-ink">{metric.value}</dd><p className="mt-3 text-xs leading-relaxed text-op-muted">{metric.note}</p></div>)}</dl>
